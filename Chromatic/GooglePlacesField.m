...
-(void)setupAutocompleteFilter {
    self.filter = [[GMSAutocompleteFilter alloc] init];
    // Filter types are defined within GMSAutoCompleteFilter.h
...
-(void)searchForPlacesNamed:(NSString *)placeName {
    ...
    [self.placeClient
     autocompleteQuery:placeName bounds:nil filter:self.filter
